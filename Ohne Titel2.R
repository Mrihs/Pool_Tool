...
server <- function(input, output, session) {
  ...
  observeEvent(input$next_question, {
    ...
    # Eine Funktion, um questions_data mit den Werten von temp_data zu aktualisieren
    update_questions_data <- function() {
      temp_data <- questions_data()  # Hole die aktuellen Daten
      
      # Aktualisiere die Werte in temp_data (so wie du es schon machst)
      temp_data$Version[current_index()] <- input$question_version %>% as.numeric()
      temp_data$Type[current_index()] <- input$type
      temp_data$Question[current_index()] <- input$question_text
      temp_data$A[current_index()] <- input$option_a
      temp_data$B[current_index()] <- input$option_b
      temp_data$C[current_index()] <- input$option_c
      temp_data$D[current_index()] <- input$option_d
      temp_data$E[current_index()] <- input$option_e
      temp_data$A_type_cor[current_index()] <- input$a_type_cor
      temp_data$A_cor[current_index()] <- input$a_cor
      temp_data$B_cor[current_index()] <- input$b_cor
      temp_data$C_cor[current_index()] <- input$c_cor
      temp_data$D_cor[current_index()] <- input$d_cor
      temp_data$Year[current_index()] <- input$year %>% as.numeric()
      temp_data$Week[current_index()] <- input$week %>% as.numeric()
      temp_data$Chapter[current_index()] <- input$chapter %>% as.numeric()
      temp_data$State[current_index()] <- input$state
      temp_data$Tags[current_index()] <- input$tags
      temp_data$Remarks[current_index()] <- input$remarks
      
      # Nun setze die Fragen-Daten (questions_data) auf die aktualisierten Werte in temp_data
      questions_data(temp_data)
    }
    
    # Sobald du die Werte aktualisieren möchtest, rufst du die Funktion auf:
    ...
    if (current_index() < SUM) {
      ...
      showNotification("INCREASE", type = "warning")
    ...
    showNotification(as.vector(total_questions()), type = "message")
    showNotification(as.vector(current_index()), type = "message")
    
    showNotification(as.vector(questions_data()[current_index(),1]), type = "message")
    # showNotification(temp_data[1,5], type = "message")
