...
server <- function(input, output, session) {
  ...
  ## 4.2 Initialize Button State ##########
  observe({
    # Update the state of the buttons at the start of the app
    shinyjs::toggleState("next_question", condition = (current_index() < total_questions()))
    shinyjs::toggleState("prev_question", condition = (current_index() > 1))
  })
  
  
  
  
  ## 4.X Function to Update Data Internally ##########
  update_questions <- function(Data, cur_ind) {
    idx <- Data[1, cur_ind]
    
    
    showNotification(idx, type = "warning")
  ...
  observeEvent(input$next_question, {
    ...
    # Speichern der Eingabewerte in questions_data
    questions_data <- questions_data()
    questions_data[idx, "Version"] <- input$question_version %>% as.numeric()
    questions_data[idx, "Type"] <- input$type
    questions_data[idx, "Question"] <- input$question_text
    questions_data[idx, "A"] <- input$option_a
    questions_data[idx, "B"] <- input$option_b
    questions_data[idx, "C"] <- input$option_c
    questions_data[idx, "D"] <- input$option_d
    questions_data[idx, "E"] <- input$option_e
    questions_data[idx, "A_type_cor"] <- input$a_type_cor
    questions_data[idx, "A_cor"] <- input$a_cor
    questions_data[idx, "B_cor"] <- input$b_cor
    questions_data[idx, "C_cor"] <- input$c_cor
    questions_data[idx, "D_cor"] <- input$d_cor
    questions_data[idx, "Year"] <- input$year %>% as.numeric()
    questions_data[idx, "Week"] <- input$week %>% as.numeric()
    questions_data[idx, "Chapter"] <- input$chapter %>% as.numeric()
    questions_data[idx, "State"] <- input$state
    questions_data[idx, "Tags"] <- input$tags
    questions_data[idx, "Remarks"] <- input$remarks
    
    # Aktualisieren der questions_data
    questions_data(questions_data)    
    ...
    shinyjs::toggleState("next_question", condition = (current_index() < total_questions()))
    shinyjs::toggleState("prev_question", condition = (current_index() > 1))
    showNotification(current_index(), type = "message")
    showNotification(total_questions(), type = "message")
